...
const Footer = () => {
  ...
  // Corrected variable name (was hideNavbarRoutes in original)
  ...
  return (
    <div className="bg-gray-900 text-white py-10 px-5 flex flex-col items-center">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-5 w-full max-w-screen-lg p-6 bg-white text-black shadow-lg rounded-lg transform -translate-y-22">
        <div>
          <h3 className="text-xl font-bold">🌟 Let's Get Started 📊</h3>
          <p className="font-semibold">Success in trading isn't about luck; it's about discipline, strategy, and constant learning.</p>
        </div>
        <button 
          onClick={() => downloadCV(Resume)} 
          className="bg-btn text-white font-bold px-6 py-3 rounded-md hover:bg-purple-500 transition duration-100"
        >
          Download Broucher
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-screen-lg mt-5">
        ...
        <div>
          ...
          <p className="text-gray-100">
            We are dedicated to shaping the future of trading by providing students with the knowledge, skills, and confidence to excel in the financial markets.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col space-y-1 uppercase">
            {['Courses','About', 'Services', 'Gallery', 'Contact'].map((link, index) => (
              <NavLink 
                key={index} 
        ...
        <div>
          ...
          <p className="text-gray-100"><strong>Address : </strong> Shree Trading Academy uttamrao shinde patil complex , Yeola 423401</p>
      ...
      <div className="w-full mt-8 text-center">
        ...
        <button onClick={() => navigate('/privacypolicy')} className="text-white hover:text-btn" >Privacy Policy</button>
        <button onClick={() => navigate('/termsandconditions')} className="text-white hover:text-btn" >Terms & Conditions</button>
        </div>
        </div>
      </div>
    </div>
